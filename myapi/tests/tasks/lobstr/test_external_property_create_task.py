...
class TestExternalPropertyCreateTask:
    class TestWithLeboncoin:
        ...
        def test_when_external_property_already_exist_it_update_it(
            ...
            assert (
                ...
                .first()
                .price
                == self.leboncoin_results[0]["price"]
            )

        @patch(
            "myapi.services.lobstr.lobstr_property_fetcher.LobstrPropertyFetcher.get_properties"
        )
        def test_when_external_property_already_exist_it_does_not_create_new_photos(
            self, mock_get_properties, db
        ):
            mock_get_properties.return_value = self.leboncoin_results
            external_property = ExternalPropertyFactory(
                source_id=self.leboncoin_results[0]["annonce_id"], source="leboncoin"
            )
            create_external_property_task("run_id", "leboncoin")
            ...
            photos = Photo.objects.filter(
                content_type=external_property_content_type,
                object_id=external_property.id,
            )
        ...
        def test_when_no_external_property_already_exist_it_creates_external_property(
            self, mock_get_properties, db
        ):
            mock_get_properties.return_value = self.leboncoin_results
            create_external_property_task("run_id", "leboncoin")
            ...
            assert (
                ExternalProperty.objects.filter(
                    ...
                    first_photo_url=self.leboncoin_results[0]["pictures"].split("|||")[
                        0
                    ],
                    floor_number=self.leboncoin_results[0]["more_details"][
                        "floor_number"
                    ],
                    ...
                    nb_parkings=self.leboncoin_results[0]["more_details"][
                        "nb_parkings"
                    ],
        ...
        def test_when_no_external_property_already_exist_it_creates_photos(
            self, mock_get_properties, db
        ):
            mock_get_properties.return_value = self.leboncoin_results
            create_external_property_task("run_id", "leboncoin")
    ...
    class TestWithPap:
        ...
        def test_when_external_property_already_exist_it_update_it(
            ...
            ExternalPropertyFactory(
                source_id=self.pap_results[0]["internal_id"], source="pap", price=123456
            )
            create_external_property_task("run_id", "pap")

            assert (
                ExternalProperty.objects.filter(
                    source_id=self.pap_results[0]["internal_id"], source="pap"
                )
                .first()
                .price
                == self.pap_results[0]["price"]
            )
        ...
        def test_when_the_LobstrPropertyFetcher_does_returns_property(
            ...
            result = create_external_property_task("run_id", "pap")
